...
in scope. This affects almost every real call this project needs to make. Point at PyPI would
mean build on a known-broken dependency. Point at a local editable checkout lets the
...
itself. Sibling repos are dependencies, not this project's surface. Only their defects
(not their feature set) are this project's problem to fix.

Every such fix must have a detailed changelog entry. The entry describes what changed and why.
