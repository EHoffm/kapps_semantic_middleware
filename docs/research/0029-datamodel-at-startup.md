...
(`class_spec.py`, `properties = expected_properties`) and returns early when the scope is empty — so
an unnamed property is an absent property, at every level.
...
construct their middleware but never serve it, so `on_start_up` never fires (that is #44).

This is **not** a defect to fix by erroring. The middleware acts on a projection; the graph holds the
information; a projection of a bare individual is legitimate and serves a one-field datamodel.
#42's `class_scope` parameter is what makes it useful. The failure surface belongs at *use* and
...
it unless the restriction declares it. Current upstream restrictions declare **no** MQTT metadata:
...
`rdfs:comment`, so strict mode fails on ground truth. Hence the targeted check in ADR 0020.
...
appear solely under inference, and `_fetch_complex_property` queries
`FROM <http://www.ontotext.com/explicit>` — so they are never fetched. The binding upstream is a
property hierarchy, already asserted:
...
repository's `inf:` vocabulary. Those superproperties currently carry **no** `rdfs:range`, which is
...
both additive extension of the fixed upstream `tu:` properties and putting ranges on the `inf:`
interface superproperties (which would collide by inheritance). RDFS ranges are conjunctive, so the
raise is a defect. **→ root ADR 0002**, plus a grilling session in `kapps_ogm` before the fix.
...
(ADR 0017). Access mode and MQTT metadata are *absent* from this tree today and enter only once the
...
passes `is_identifiable_type`, and `ClassSpec.to_pydantic_model` builds on plain `pydantic.BaseModel`
— so belts and barriers are invisible to the generator, not merely the speeds. ADR 0017's premise
("the deepest addressable thing is the whole belt list") is corrected there: the depth is **zero**,
and #41 inherits nothing.

## Hygiene issues found

- **Stale evidence.** `kapps_ogm/scripts/output/` was last written at `ef74709`, which **pre-dates**
  `eb09f3d` (the `SCOPE` early-return). Those committed files show a shape the current code does not
  produce — #29's amendment cited them as ground truth and should not have.
- **Dirty `OGM` repository.** Only `tui:LightBarrier1_back` has the correct blanknode shape.
  ...
  **flattened to literals** (`tu:hasConveyorSpeed → 10.2`), which is why Case C returned `[]` for
  them — `?bnode ?property ?value` cannot match a literal. Leftover `inf:isAttribute` /
