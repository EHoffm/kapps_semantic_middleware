...
| **Transactional-write discipline** (standing constraint: every write a transaction; precondition-checks *before* it; revert on failure) | Partially implicit. The atomic `DELETE/INSERT` exists in `graph_db_interface`. The middleware doesn't yet systematically check-then-transact (there's no possession switch, no handover txn) | Systematic: checks outside, one clean revertible transaction inside | discipline not yet a code pattern | **ADD (enforce)** |
