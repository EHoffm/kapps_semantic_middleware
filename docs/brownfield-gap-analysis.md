...
wayfinder tickets say it should be*. Seed a brownfield `grill → PRD → issues` pass. This
...
`connectors/`, `shacl_interop/`, and the Core Middleware ADRs (0001–0008). Also the resolved
...
[#6 layout](https://github.com/EHoffm/kapps_semantic_middleware/issues/6), and
...
The facade's **registration / execution-resolution / liveness** half exists. It largely
matches the intent. The entire **coordination + handover** half that Map #2 designed **does not
exist yet**. The one piece that *looks* built — `execute()` — implements the **old
synchronous-RPC model**. Not the decided doorbell/queue/pull model. This is not a case of adding the
...
| **Transactional-write discipline** (standing constraint: every write a transaction; precondition-checks *before* it; revert on failure) | Partially implicit. The atomic `DELETE/INSERT` exists in `kapps_triplestore_interface`. The middleware doesn't yet systematically check-then-transact (there's no possession switch, no handover txn) | Systematic: checks outside, one clean revertible transaction inside | discipline not yet a code pattern | **ADD (enforce)** |
...
stops at **0008**. It contains none of them. A stateful `grill-with-docs` pass should
**promote the confirmed decisions into ADRs** (0009+ / a new MES-layering ADR). `CONTEXT.md`
and the ADRs are the ground truth the builders read. Not just the issue tracker.
...
tests exercise `execute()`** in its current synchronous-RPC form. The `execute()` refactor
will break them. They encode the *old* model. Rewrite them alongside it. Liveness and
shape-from-typehints tests are orthogonal. They should survive.

## 5. Suggested build sequence (a *seed* for grilling, not a decision)

Dependency order the grilling should pressure-test. Do not adopt uncritically:
...
2. **Coordination machinery** — this bundles the `execute()` refactor, operation queue, status lifecycle, domain callback, `pop_next(class_scope)`, and failed-op state dump. It is the load-bearing cluster. The rest hangs off it.
...
- **Migration safety**. Rebuild the facade in place on a branch. Or find a parallel path, so the working registration/liveness half keeps passing while `execute()` is reworked.
- **ADR promotion**. Which decided tickets become ADRs? Do any existing ADRs (esp. 0002/0005) need amending to reflect the doorbell model?

---

*This document is ground truth for a brownfield `grill → PRD → issues` pass. Open the grilling
in a fresh session. Reference this file. Do not re-derive the decisions already recorded in
