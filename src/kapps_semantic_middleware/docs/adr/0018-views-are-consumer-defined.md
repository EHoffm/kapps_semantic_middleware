...
**rooted at the node that the consumer cares about**. It states this scope in the code that embeds
the middleware library. Do not state it in the ontology. `SemanticMiddleware` gains a `class_scope` constructor
parameter carrying the **user view**. This is the northbound datamodel it materializes and REST-exposes.
...
bypass the middleware and drive the PLC over MQTT directly. This is exactly the coupling the architecture
exists to remove.
...
> every peer. It also argued that the *view* prevents it by not projecting it. Both halves were wrong.
>
...
known primitives**. A task made of grip/move/place may combine them in a way nobody saw before,
but grip, move, and place are each known. A product may combine a screw and a gear that were each known.
...
UI renders an input where it finds `readwrite` and a read-only row where it finds `read`. The
route generator gates the PUT verb off the same field (ADR 0017). ~~A static facet costs nothing
here, since `OGM.commit` filters unchanged triples.~~
...
No SPARQL is required, because the middleware and the OGM wrote that metadata and know what it means.
...
- Omitting `class_scope` falls back to today's scopeless `ogm.fetch(..., materialize=True)`, so
  ...
  middleware acts on a projection. The knowledge graph holds the information. A projection of a
...
- A consumer that opens a resource **GETs its REST datamodel** and renders the returned tree. It does
  not fetch structure from the graph under a view of its own. The view is stated once, by the
...
- Views defined in Python are invisible to the graph. Nothing therefore validates that a view matches what
